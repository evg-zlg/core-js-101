...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (bigChars.includes(str[i])) {
      result += bigChars[(bigChars.indexOf(str[i]) + 13) % 26];
    } else if (smallChars.includes(str[i])) {
      result += smallChars[(smallChars.indexOf(str[i]) + 13) % 26];
    } else {
      result += str[i];
    }
  }
